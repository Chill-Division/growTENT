...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST['plantid'])) {
		// Reprint label
    $new_plant_uniqueid = uniqid('p', true);
		$fileName = $new_plant_uniqueid . ".png";
		$pngAbsoluteFilePath = 'qrcodes/' . $fileName;
		if (!file_exists($pngAbsoluteFilePath)) {
			QRcode::png($new_plant_uniqueid, $pngAbsoluteFilePath, QR_ECLEVEL_L, 4, 2);

			$fileToDelete = 'qrcodes/' . $_POST['plantid'] . '.png';
		} else {
			echo "File already generated!";
		}
		// DB operation
		$sql = "UPDATE inventory SET plant_uniqueid = '$new_plant_uniqueid' WHERE plant_uniqueid = '{$_POST['plantid']}'";
		if ($result = mysqli_query($con, $sql)) {
			echo "<table style='font-family: monospace; padding: 0;'>
				<tr>
					<td rowspan='5'><img src='" . $fileToDelete . "' width='80' height='80' /></td>
					<td>Plant #" . 'Old Label' . "</td>
					<td style='padding-left: 30px; padding-right: 30px'> | </td>
					<td rowspan='5'><img src='" . $pngAbsoluteFilePath . "' width='80' height='80' /></td>
					<td>Plant #" . 'New Label' . "</td>
				</tr>
				<tr>
					<td>Reprint date: " . $date . "</td>
					<td style='padding-left: 30px; padding-right: 30px'> | </td>
					<td>Reprint date: " . $date . "</td>
				</tr>
				<tr>
